...
import timeFormat from '../lib/timeFormat' // Fixed import
...
const MovieDetails = () => {
  ...
  return show ? (
    <div className='px-6 md:px-16 lg:px-40 md:pt-16'>
      <div className='flex flex-col md:flex-row gap-8 max-w-6xl mx-auto'>
        <img
          src={show.movie.poster_path || "/fallback-poster.png"} // optional fallback image
          alt={show.movie.title || "Movie Poster"}
          className='max-md:mx-auto rounded-xl h-96 max-w-[280px] object-cover'
        />
        ...
        <div className='relative flex flex-col gap-3'>
          ...
          <p>
            {timeFormat(show.movie.runtime)} • {show.movie.genres?.map(genre => genre.name).join(", ") || "No genres"} •
        ...
        <div className='flex items-center gap-4 w-max px-4'>
          {show.movie.casts?.slice(0, 12).map((cast, index) => (
            <div key={index} className='flex flex-col items-center text-center'>
              <img
                src={cast.profile_path || "/fallback-cast.png"} // optional fallback for cast
                alt={cast.name || "Cast"}
                className='rounded-full h-32 w-32 aspect-square object-cover object-center'
              />
              <p className='font-medium text-xs mt-3'>{cast.name || "Unknown"}</p>
            </div>
